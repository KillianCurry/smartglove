...
//quaternion components (identity 0,0,0,1)
float q[4] = {0,0,0,1.0};
float samplePeriod = 1.0 / (float)FREQUENCY_IMU_NEW_SAMPLE;
float beta = 0.1;
int RawDataIMU[20];
#endif
...
void setup() {
  ...
  //UUID SPECIFICATIONS
  //first digit: IMU (0 = none, 6 = 6DoF)
  //next two digits: sensor count (10 = 10-sensor, 05 = 5-sensor)
  //next three digits: unique ID
#ifdef ENABLE_IMU
  String imuString = "6";
#else
  String imuString = "0";
  ...
  //name glove
  blePeripheral.setLocalName(("StretchSense Glove " + idString).c_str()); //broadcast device name
  blePeripheral.setDeviceName(("StretchSense Glove " + idString).c_str());
  //store front and back of string, middle is service/characteristic identifier
  String UUIDfront = imuString + chanString + idString;
  String UUIDback = "-7374-7265-7563-6873656e7365";
  stretchSenseService = BLEService((UUIDfront + "01" + UUIDback).c_str());
  stretchSenseChar = BLECharacteristic((UUIDfront +"02" + UUIDback).c_str(), BLERead | BLENotify, 20);
  imuChar = BLECharacteristic((UUIDfront + "50" + UUIDback).c_str(), BLERead | BLENotify, 20);
...
void loop() {
  ...
  if (microsNow - microsPrevious_IMU >= microsPerReading_IMU) {
    ...
    // (add 360 to every one to avoid negative values)
    roll_int = (filter.getRoll() + 360) * 100;
    pitch_int = (filter.getPitch() + 360) * 100;
    yaw_int = (filter.getYaw() + 360) * 100;
    accel_x_int = ax * 100;
    accel_y_int = ay * 100;
    accel_z_int = az * 100;
  
    // separate each integer IMU into 2 hexadecimal values
    RawDataIMU[0] = roll_int / 256;
    RawDataIMU[1] = roll_int - ((roll_int / 256) << 8);
    RawDataIMU[2] = pitch_int / 256;
    RawDataIMU[3] = pitch_int - ((pitch_int / 256) << 8);
    RawDataIMU[4] = yaw_int / 256;
    RawDataIMU[5] = yaw_int - ((yaw_int / 256) << 8);
